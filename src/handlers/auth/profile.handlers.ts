...
import { IDataUpdateProfileResponse, IDetailDataResponse } from "../../models/auth/profile.model";
import { cloudinarySingleUploader } from "../../helpers/cloudinary";

export const Update = async (req: Request, res: Response<IDataUpdateProfileResponse>) => {
  try {
    const { file } = req;
    const { id } = req.params;
